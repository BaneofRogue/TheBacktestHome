...
loadBtn.addEventListener('click', async () => {
    ...
    chart.raw1mData = newData;          // save original 1m data
...
// Handle real-time timeframe change
timeframeSelect.addEventListener('change', () => {
    const timeframe = parseInt(timeframeSelect.value, 10);
    if (chart.raw1mData.length > 0) {
        chart.loadCandles(chart.raw1mData, timeframe); // aggregate from saved 1m
    }
});
